...
const UserPanel = () => {
  ...
  useEffect(() => {
    ...
    const storedPurchasedProjects = localStorage.getItem('purchasedProjects');
    if (loggedInUser) {
    ...
    if (storedPurchasedProjects) {
      setPurchasedProjects(JSON.parse(storedPurchasedProjects));
    }
  }, []);
        ...
        <div className="flex">
          <div className="w-1/4 p-4 bg-gray-100 dark:bg-gray-800 rounded-lg shadow">
            <ul className="space-y-4">
              ...
              <li className={`cursor-pointer text-lg font-medium ${selectedOption === 'purchasedProjects' ? 'text-blue-600' : 'text-gray-700 dark:text-gray-300'}`} onClick={() => handleOptionSelect('purchasedProjects')}>Purchased Projects</li>
            </ul>
          </div>
          <div className="w-3/4 p-4 bg-white dark:bg-gray-900 rounded-lg shadow ml-4">
            ...
            {selectedOption === 'makeSitePlan' && (
              <div>
                ...
                <a href="https://www.requestaplan.co.uk/?gad_source=1&gclid=Cj0KCQjwu8uyBhC6ARIsAKwBGpRQZ73imV_ouEX7syimF-poE6oBah-cAwWKAdnXCDogZFyhJxCK8YcaAvX8EALw_wcB" className="text-blue-600 cursor-pointer underline" target="_blank" rel="noreferrer">ReQuestaPlan</a>
                ...
                <div className="flex items-center mt-4">
                  ...
                  <div>
                    ...
                    <button
                      className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:focus:ring-blue-800 transition-all duration-300"
            ...
            {selectedOption === 'purchasedProjects' && (
              <div>
                <h4 className="text-xl font-medium text-gray-900 dark:text-white mb-4">Purchased Projects:</h4>
                {purchasedProjects.length > 0 ? (
                  <ul className="divide-y divide-gray-200">
                    {purchasedProjects.map((purchase, index) => (
                      <li key={index} className="py-4">
                        <h5 className="text-lg font-bold text-gray-900 dark:text-gray-200 mb-2">Invoice Number: {purchase.invoiceNumber}</h5>
                        <ul>
                          {purchase.projects.map((project, projectIndex) => (
                            <li key={projectIndex} className="flex items-center mb-2">
                              <img src={project.image} alt={project.title} className="w-20 h-20 mr-4 object-cover rounded-lg" />
                              <div>
                                <h5 className="text-lg font-bold text-gray-700 dark:text-gray-300">{project.title}</h5>
                                <p className="text-gray-900 dark:text-gray-200">{project.description}</p>
                                <p className="text-lg font-bold text-gray-900 dark:text-gray-200">£{project.price.toFixed(2)}</p>
                        ...
                        <p className="text-lg font-bold text-gray-900 dark:text-gray-200">Total Price: £{purchase.totalPrice.toFixed(2)}</p>
                      </li>
                    ))}
                  </ul>
                ) : (
                  <p className="text-gray-700 dark:text-gray-300">No purchased projects found.</p>
                )}
              </div>
            )}
            <div className="mt-10">
